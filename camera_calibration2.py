...
def extract_corners(image, bg_model):
    ...
    _, chessboard_binary_white = cv2.threshold(chessboard_gray, 150, 255, cv2.THRESH_BINARY)
    
    #-----------------------------------------
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5))
    chessboard_binary_white = cv2.morphologyEx(chessboard_binary_white,cv2.MORPH_OPEN,kernel)
    
    contours, _ = cv2.findContours(chessboard_binary_white, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    # Sort contours by area in descending order and take the largest contour
    contours = sorted(contours, key=cv2.contourArea, reverse=True)
    largest_contour = contours[0]

    # Approximate the contour to a polygon
    perimeter = cv2.arcLength(largest_contour, True)
    polygon = cv2.approxPolyDP(largest_contour, 0.02 * perimeter, True)

    # Create mask
    chessboard_mask = np.zeros_like(chessboard_gray)
    cv2.drawContours(chessboard_mask, [polygon], -1, (255), thickness=cv2.FILLED)
    ...
    _, chessboard_binary_black = cv2.threshold(chessboard_gray, 160, 255, cv2.THRESH_BINARY_INV)
    black_squares = cv2.bitwise_and(chessboard_binary_black, chessboard_binary_black, mask=chessboard_mask)
    ...
    # Resize images
    resized_chessboard_gray = cv2.resize(chessboard_gray, (new_width, new_height))
    resized_chessboard_binary_white = cv2.resize(chessboard_binary_white, (new_width, new_height))
    resized_chessboard_cropped = cv2.resize(chessboard_cropped, (new_width, new_height))
    resized_chessboard_binary_black = cv2.resize(chessboard_binary_black, (new_width, new_height))
    resized_black_squares = cv2.resize(black_squares, (new_width, new_height))

    # Concatenate images horizontally
    hori = np.concatenate((resized_chessboard_gray, resized_chessboard_binary_white, resized_chessboard_cropped, resized_chessboard_binary_black, resized_black_squares), axis=1)
